...
class DemoSpringSecurityApplicationTests {
	...
	void GetWithAuthorizedUser() throws Exception {
		...
		MvcResult mvcResult = mockMvc.perform(formLogin().user("john").password("pwd")).andReturn();
		String accessToken = mvcResult.getResponse().getHeader("access_token");
		mockMvc.perform(get("/api/users").header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken))
	...
	void GetWithUnAuthorizedUser() throws Exception {
		...
		MvcResult mvcResult = mockMvc.perform(formLogin().user("paul").password("pwd")).andReturn();
		String accessToken = mvcResult.getResponse().getHeader("access_token");
		mockMvc.perform(get("/api/users").header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken))
	...
	void CreateANewUserWithAuthorizedUser() throws Exception {
		...
		MvcResult mvcResult = mockMvc.perform(formLogin().user("dave").password("pwd")).andReturn();
		String accessToken = mvcResult.getResponse().getHeader("access_token");
		...
		mockMvc.perform(
				post("/api/users").contentType(MediaType.APPLICATION_JSON).content(requestJson).header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken))
				.andExpect(status().isOk());
	...
	void GetUserByUserName() throws Exception {
		MvcResult mvcResult = mockMvc.perform(formLogin().user("john").password("pwd")).andReturn();
		String accessToken = mvcResult.getResponse().getHeader("access_token");
		mockMvc.perform(get("/api/users?username=john").header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken)).andExpect(status().isOk());
	...
	void addUserToRole() throws Exception {
		MvcResult mvcResult = mockMvc.perform(formLogin().user("dave").password("pwd")).andReturn();
		String accessToken = mvcResult.getResponse().getHeader("access_token");
		mockMvc.perform(post("/api/users/john/roles/MANAGER").header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken))
				.andExpect(status().isOk());
